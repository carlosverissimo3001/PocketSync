  ...
  async handleHandoff(job: Job) {
    ...
    try {
      const originalPrisma =
        this.shardRouterService['shardClients'][originalShard];
      if (!originalPrisma)
        throw new Error(`Original shard ${originalShard} not found`);

      // Perform the operation on the original shard
      await originalPrisma[model][operation](data);

      this.logger.log(
        `Handoff successful: Operation ${operation} on model ${model} for data ${JSON.stringify(data)} moved back to ${originalShard}`,
      );
    } catch (error: unknown) {
      if (error instanceof Error) {
        this.logger.error(
          `Handoff failed for shard ${originalShard}: ${error.message}`,
        );
      } else {
        this.logger.error(
          `Handoff failed for shard ${originalShard}: ${JSON.stringify(error)}`,
        );
      }
